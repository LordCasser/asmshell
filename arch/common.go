...
func SetArch(strArch string) Machine {
	...
	case "x64":
		mc = SetX64(strArch)
	case "thumb":
		mc = SetArmThumb(strArch, false)
	case "thumbbe":
		mc = SetArmThumb(strArch, true)
	case "arm":
		mc = SetArm(strArch, false)
	case "armbe":
		mc = SetArm(strArch, true)
	case "arm64":
		mc = SetArm64(strArch, false) // TODO: test, fixme: something wrong?
	//case "arm64be"     : mc = SetArm64(strArch, true)     // keystone UNSUPPORTED?, unicorn: supported, fixme: something wrong?

	case "mips":
		mc = SetMips(strArch, false)
	case "mipsbe":
		mc = SetMips(strArch, true)
	case "mips64":
		mc = SetMips64(strArch, false) // fixme: something wrong?
	case "mips64be":
		mc = SetMips64(strArch, true) // fixme: something wrong?
	case "sparc":
		mc = SetSparc(strArch, true) // sparc standard is big-endian
	case "sparcle":
		mc = SetSparc(strArch, false) // assemble only, unicorn: UNSUPPORTED, keystone: supported
	case "sparc64":
		mc = SetSparc64(strArch, true) // fixme: something wrong?, sparc standard is big-endian, keystone sparc64el does not supported
	case "ppc", // big-endian, assemble only
		"powerpc":
		mc = SetPowerPC(strArch, true)
	case "ppc64", // assemble only
		"powerpc64":
		mc = SetPowerPC64(strArch, true)
	case "ppc64le", // assemble only
		"powerpc64le":
		mc = SetPowerPC64(strArch, false)
	case "sysz",
		"systemz",
		"systemZ":
		mc = SetSystemZ(strArch)
...
// IshellRun assemble and emulate(for ishell)
func (mc Machine) IshellRun(c *ishell.Context, mnemonic string) error {
	...
	if mc.mu == nil { // if unicorn does not supported
		c.Printf("mnemonic: %s don't be supported by unicorn\n", mnemonic)
		return nil
	}
...
// Run assemble and emulate only does not output
func (mc Machine) Run(mnemonic string) error {
	code, err := mc.Assemble(mnemonic)
	if err != nil {
		return err
	}
	if mc.mu == nil { // if unicorn does not be supported
		return nil
	}
	if err := mc.Emulate(code); err != nil {
		return err
...
func (mc Machine) Emulate(code []byte) error {
	...
	if strings.Contains(mc.Prompt, "thumb") { // arm thumb mode
		...
		err := mc.mu.MemWrite(uint64(mc.start-1), code)
		if err != nil {
			return err
		}
	} else {
		codeEnd = uint64(mc.start) + uint64(len(code))
		err := mc.mu.MemWrite(uint64(mc.start), code)
		if err != nil {
			return err
		}
	}
...
func (mc Machine) showStatusReg(c *ishell.Context, key string, reg uint64, oldReg uint64) {
	...
	switch key {
	case "eflags":
		flags = map[string]uint{"CF": 0, "PF": 2, "AF": 4, "ZF": 6, "SF": 7, "IF": 9, "DF": 10, "OF": 11} // intel
	...
	case "cpsr":
		flags = map[string]uint{"T": 5, "E": 9, "Q": 27, "V": 28, "C": 29, "Z": 30, "N": 31} // arm
		order = []string{"T", "E", "Q", "V", "C", "Z", "N"}
	case "nzcv":
		flags = map[string]uint{"V": 28, "C": 29, "Z": 30, "N": 31} // arm64, oVerflow/Carry out/Zero/Negative
